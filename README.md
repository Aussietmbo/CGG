# CGG Research and Simulators

A collection of physics simulators and computational tools focused on compact object physics, field theory, and coherence phenomena. All simulators are open for public use and collaboration.

## 🎯 Featured Simulators

Quick access to our most interactive and visually striking tools:

- **[3D Fourier Synthesis v5.0](simulators/wave-dynamics/fourier_synthv5.3.html)** — Wave builder with resonance manifold visualization
- **[Coherence-Driven Cellular Automata v4.2](simulators/cellular-automata/coherence-driven-ca_fft_v4_2.html)** — Phase diagram explorer with spectral feedback
- **[Lambdoma Resonance Keyboard](simulators/harmonic-resonance/lambdoma-resonance-autochord_modulated.html)** — Interactive harmonic tuning system
- **[Resonant Spheres Simulator](simulators/wave-dynamics/resonant-spheres-sim_v6.1.html)** — Interference pattern and focus analysis
- **[CGG Vortex-Core Black Hole](simulators/trr-g-core/blackhole_cgg_vortex.html)** — Compact object interior vortex dynamics

---

## 📁 Simulator Categories

### [TRR-G Compact Object Simulators](simulators/trr-g-core/)
Core simulators for studying compact object physics, vortex formation, and gravitational effects.
- TRR-G Paper III — Vortex Nucleation Simulator
- TRR-G / CGG — Light Propagation Simulator
- TRR-G — ψ Wavefront Propagation
- TRR-G Vortex Field Explorer — WebGL
- CGG Vortex-Core Black Hole

### [Field Solvers & Diagnostics](simulators/field-solvers-diagnostics/)
Numerical solvers and diagnostic tools for field equations and validation.
- TRR-G Radial Solver — Coherence-Modified Gravitational Potential
- TRR-G κ Isolation Test — Mismatch Diagnostics
- TRR-G 3D κ Field Simulator
- TRR-G Substrate Evaluator v2 — Dynamic Field Evolution
- TRR-G Klein–Gordon 1D (flipped) — 1‑D kink solver with flipped boundary conditions

### [Wave Dynamics & Resonance](simulators/wave-dynamics/)
Simulators for wave propagation, resonance phenomena, and field dynamics.
- TRR Wave Tank (GPU) – Dot-Product Coherence
- 3D Fourier Synthesis v5.0 - Wave Builder + Resonance Manifold
- Resonant Spheres — Interference Focus (v4.3)
- TRR-G: Solar System Coherence Field

### [Cellular Automata](simulators/cellular-automata/)
Coherence-driven cellular automaton with spectral feedback and parameter exploration.
- Coherence-Driven Cellular Automata v4.2

### [Harmonic & Musical Resonance](simulators/harmonic-resonance/)
Exploratory tools for harmonic resonance and musical physics.
- The Big Chord — Primordial Resonance Simulator
- Lambdoma Resonance Keyboard

### [Visualization & Interactive Tools](simulators/visualization-tools/)
Advanced visualization platforms for interactive exploration of field data.
- TRR-G V12 Visual Simulator (Three.js, No External Controls)

---

## 🚀 Getting Started

Each simulator folder contains:
- A detailed README with usage instructions
- Source code and documentation
- Links to related research papers

Select a category above to explore the simulators and their documentation.

---

## 📚 Research & Publications

### 📄 [Download: TRR_CGG_V27](papers/TRR_CGG_V27.pdf)

Comprehensive TRR-G Theory Document (March 2026)

The complete research paper covering theoretical framework, compact object physics, gravitational effects, and wave propagation.

All simulators are validated against peer research. Details and citations are available in each simulator's documentation.

**[View papers folder →](papers/)**

---

## 📧 Contact

For questions, contributions, or collaborations, please open an issue or contact the repository maintainers.

*Last updated: March 2026*
